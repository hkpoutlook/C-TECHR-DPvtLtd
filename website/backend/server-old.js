...
// Define categories
const CATEGORIES = {
  foundation: { id: 1, name: 'Foundation', level: 'Beginner', access: 'free', color: '#4CAF50' },
  professional: { id: 2, name: 'Professional', level: 'Intermediate', access: 'paid', color: '#2196F3' },
  research: { id: 3, name: 'Research', level: 'Advanced', access: 'paid', color: '#FF9800' }
};
...
app.get('/api/categories/:categoryKey', (req, res) => {
  const category = CATEGORIES[req.params.categoryKey];
  if (!category) {
...
const books = {
  foundation: [
    { id: 1, title: 'Foundations of Electronics & Measurement', level: 'Foundation', access: 'free', author: 'C-TECH Team' },
    { id: 2, title: 'Sensor Basics for Engineers', level: 'Foundation', access: 'free', author: 'C-TECH Team' },
    { id: 3, title: 'Understanding Signals – A Conceptual Approach', level: 'Foundation', access: 'free', author: 'C-TECH Team' }
  ],
  professional: [
    { id: 4, title: 'Practical Instrumentation System Design', level: 'Professional', access: 'paid', author: 'C-TECH Team' },
    { id: 5, title: 'Embedded Systems for Industrial Applications', level: 'Professional', access: 'paid', author: 'C-TECH Team' },
    { id: 6, title: 'Sensor Signal Conditioning – Field Proven Methods', level: 'Professional', access: 'paid', author: 'C-TECH Team' }
  ],
  research: [
    { id: 7, title: 'Advanced Precision Measurement Systems', level: 'R&D', access: 'paid', author: 'C-TECH Team' },
    { id: 8, title: 'R&D Engineering: From Problem to Innovation', level: 'R&D', access: 'paid', author: 'C-TECH Team' },
    { id: 9, title: 'Intelligent Sensors & AI-Assisted Instrumentation', level: 'R&D', access: 'paid', author: 'C-TECH Team' }
  ]
};
...
app.get('/api/concept-books/:category', (req, res) => {
  const categoryBooks = books[req.params.category];
  if (!categoryBooks) {
    return res.status(404).json({ error: 'Category not found' });
  }
  res.json({ category: req.params.category, books: categoryBooks });
});
...
const components = {
  foundation: {
    ...
    access: 'free',
    items: [
      { id: 1, name: 'Load Cell - Basic', type: 'sensor', price: 500 },
      { id: 2, name: 'Temperature Sensor - LM35', type: 'sensor', price: 300 },
      { id: 3, name: 'Pressure Sensor - BMP180', type: 'sensor', price: 400 }
    ]
  },
  professional: {
    ...
    access: 'paid',
    items: [
      { id: 4, name: 'Industrial Load Cell - 50kg', type: 'sensor', price: 8000 },
      { id: 5, name: 'ADS1256 ADC', type: 'adc', price: 3000 },
      { id: 6, name: 'STM32F4 Discovery', type: 'microcontroller', price: 2500 }
    ]
  },
  research: {
    ...
    access: 'paid',
    items: [
      { id: 7, name: 'Ultra-low-noise Amplifier', type: 'amplifier', price: 15000 },
      { id: 8, name: 'Multi-channel DAQ System', type: 'daq', price: 25000 },
      { id: 9, name: 'AI-ready Edge Processor', type: 'processor', price: 12000 }
    ]
  }
};
...
app.get('/api/components/:category', (req, res) => {
  const categoryComponents = components[req.params.category];
  if (!categoryComponents) {
    return res.status(404).json({ error: 'Category not found' });
  }
  res.json({ category: req.params.category, components: categoryComponents });
});
      access: 'paid'
    },
    research: {
